...
export interface ModernInputProps extends Omit<TextInputProps, 'style'> {
  ...
  leftIcon?: string;
  rightIcon?: string;
  prefix?: string; // Added prefix prop support
...
export const ModernInput = forwardRef<TextInput, ModernInputProps>(
  (
    ...
    const getBorderColor = () => {
      ...
      return isDark ? 'rgba(255, 255, 255, 0.2)' : colors.border;
    };
    ...
    const styles = StyleSheet.create({
      ...
      inputWrapper: {
        ...
        backgroundColor: variant === 'filled'
          ...
          : (variant === 'glass' ? 'transparent' : (isDark ? colors.surface : colors.white)),
        height: getHeight(),
      ...
      prefix: {
        ...
        marginRight: 2,
        marginTop: floatingLabel ? 12 : 0, // Align with input text when label floats
        fontWeight: '500',
      },
      label: {
        position: 'absolute',
      ...
      <View style={styles.container}>
        ...
        {(error || hint || (showCharacterCount && maxLength)) && (
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={{ flex: 1 }}>
              {error ? (
                <Text style={[styles.supportText, { color: colors.error }]}>{error}</Text>
              ) : hint ? (
